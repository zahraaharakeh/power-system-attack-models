...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch_size, input_dim) - we need to add sequence dimension
        batch_size = x.size(0)
...
class AdvancedAttentionAutoencoder(nn.Module):
    """Advanced auto-encoder with multi-head attention and residual connections"""
    def __init__(self, input_dim, hidden_dims, latent_dim, attention_dim=128, num_heads=4):
        super(AdvancedAttentionAutoencoder, self).__init__()
        ...
        encoder_layers.append(MultiHeadAttentionLayer(prev_dim, attention_dim, num_heads))
        
        # Latent space with additional processing
        encoder_layers.extend([
    ...
    def encode(self, x):
        ...
        # Apply attention directly (no need to reshape since we fixed the attention layer)
...
def generate_advanced_malicious_data(benign_data):
    ...
    feature_mean = np.mean(benign_data, axis=0)
...
def train_advanced_attention_autoencoder(model, train_loader, val_loader, epochs=100, lr=0.001):
    """Train the advanced attention auto-encoder with improved training"""
...
def evaluate_advanced_attention_autoencoder(model, test_loader, threshold_percentile=95):
    """Evaluate the advanced attention auto-encoder with ensemble threshold optimization"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.eval()
...
def save_advanced_attention_autoencoder_results(results, model_name="Advanced Attention Autoencoder"):
    """Save results to text file"""
    print("Saving results...")
    
    with open('advanced_attention_autoencoder_results.txt', 'w') as f:
        f.write("Advanced Attention Autoencoder Model Results\n")
        f.write("=" * 50 + "\n\n")
    ...
    print("Results saved to 'advanced_attention_autoencoder_results.txt'")

def main():
    """Main function to run the complete Advanced Attention Autoencoder analysis."""
    print("Advanced Attention Autoencoder Model for Power System Attack Detection")
    print("=" * 70)
    ...
    input_dim = X.shape[1]
    model = AdvancedAttentionAutoencoder(
        input_dim=input_dim,
    ...
    # Train model
    print("\nTraining Advanced Attention Autoencoder...")
    train_losses, val_losses = train_advanced_attention_autoencoder(
        model, train_loader, val_loader, epochs=1000, lr=0.001
    )
    
    # Evaluate model
    print("\nEvaluating Advanced Attention Autoencoder...")
    results = evaluate_advanced_attention_autoencoder(model, test_loader)
    
    # Print results
    print(f"\nAdvanced Attention Autoencoder Results:")
    ...
    # Save results
    save_advanced_attention_autoencoder_results(results)
    
    # Save model
    torch.save(model.state_dict(), 'advanced_attention_autoencoder_model.pth')
    print("Advanced model saved as 'advanced_attention_autoencoder_model.pth'")
    
    print("\n" + "="*70)
    print("ADVANCED ATTENTION AUTOENCODER ANALYSIS COMPLETE!")
    print("="*70)
    print("Files generated:")
    print("- advanced_attention_autoencoder_results.txt (detailed results)")
    print("- advanced_attention_autoencoder_model.pth (trained model)")
