...
class SimpleAttentionAutoencoder(nn.Module):
    """Simple Auto-encoder with attention mechanism for power system attack detection"""
    def __init__(self, input_dim, hidden_dim=128, latent_dim=32, attention_dim=64):
        super(SimpleAttentionAutoencoder, self).__init__()
    ...
    def encode(self, x):
        ...
        # Apply attention
        attended, attention_weights = self.attention_forward(encoded)
...
def generate_malicious_data(benign_data):
    """Generate malicious data with realistic attack patterns."""
    print("Generating malicious data...")
    
    # 1. False Data Injection (FDI)
    fdi_attack = benign_data + np.random.normal(0, 0.1, benign_data.shape)
    
    # 2. Replay Attack
    replay_attack = benign_data + np.random.normal(0, 0.1, benign_data.shape)
    
    # 3. Covert Attack
    covert_attack = benign_data + np.random.normal(0, 0.1, benign_data.shape)
    
    # 4. Realistic Noise Attack
    realistic_noise = benign_data + np.random.normal(0, 0.1, benign_data.shape)
...
def evaluate_autoencoder(model, test_loader, threshold_percentile=95):
...
def plot_results(train_losses, val_losses, results):
    ...
    plt.tight_layout()
    plt.savefig('simple_attention_autoencoder_results.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("Visualizations saved as 'simple_attention_autoencoder_results.png'")

def save_results(results):
    """Save results to text file"""
    print("Saving results...")
    
    with open('simple_attention_autoencoder_results.txt', 'w') as f:
        f.write("Simple Attention Autoencoder Model Results\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write("- Auto-encoder with encoding/decoding\n")
        ...
        f.write("1. False Data Injection (FDI)\n")
        f.write("2. Replay Attack\n")
        f.write("3. Covert Attack\n")
        f.write("4. Realistic Noise Attack\n")
    
    print("Results saved to 'simple_attention_autoencoder_results.txt'")

def main():
    """Main function to run the complete Simple Attention Autoencoder analysis."""
    print("Simple Attention Autoencoder Model for Power System Attack Detection")
    print("=" * 60)
    ...
    # Create model
    input_dim = X.shape[1]
    model = SimpleAttentionAutoencoder(
        input_dim=input_dim,
        hidden_dim=128,
        latent_dim=32,
        attention_dim=64
    ...
    # Train model
    print("\nTraining Simple Attention Autoencoder...")
    train_losses, val_losses = train_autoencoder(
        model, train_loader, val_loader, epochs=50, lr=0.001
    )
    
    # Evaluate model
    print("\nEvaluating Simple Attention Autoencoder...")
    results = evaluate_autoencoder(model, test_loader)
    
    # Print results
    print(f"\nSimple Attention Autoencoder Results:")
    ...
    # Save model
    torch.save(model.state_dict(), 'simple_attention_autoencoder_model.pth')
    print("Model saved as 'simple_attention_autoencoder_model.pth'")
    
    print("\n" + "="*60)
    print("SIMPLE ATTENTION AUTOENCODER ANALYSIS COMPLETE!")
    print("="*60)
    print("Files generated:")
    print("- simple_attention_autoencoder_results.png (visualizations)")
    print("- simple_attention_autoencoder_results.txt (detailed results)")
    print("- simple_attention_autoencoder_model.pth (trained model)")
